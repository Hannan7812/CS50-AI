...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        person_prob *= person_prob * PROBS["trait"][person_gene][person_trait]
...
def normalize(probabilities):
    ...
    """
    for person in probabilities:
        gene_fac = sum(probabilities[person]["gene"].values())
        trait_fac = sum(probabilities[person]["trait"].values())

        probabilities[person]["gene"] = {genes : (prob / gene_fac) for genes, prob in probabilities[person]["gene"].items()}
        probabilities[person]["trait"] = {traits : (prob / trait_fac) for traits, prob in probabilities[person]["trait"].items()}
